...
lazy_static! {
    static ref CARD_RE: Regex = Regex::new(r"^\s*Card\s*(?P<id>\d*):(?P<win>[^|]*)\|(?P<pick>[^|]*)$").unwrap();
...
impl Card {
    ...
    fn new(line: &str) -> Card {
        ...
        Card {
            _id: captures.name("id").unwrap().as_str().parse::<usize>().unwrap()-1,
    ...
    fn get_points(&self) -> i32 {
        ...
        if matches == 0 { 0 } else { 2i32.pow((matches-1) as u32) }
    }
}

pub fn part1() {
    let p1: i32 = Card::all_cards(INPUT_DAY04).into_iter().map(|c| c.get_points()).sum();
    println!("part1: {p1}")
}
